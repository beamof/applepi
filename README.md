...
embeddings:
  model: bge-small-zh-v1.5      # 本地 embedding 模型（candle 纯 Rust 推理，离线）
  # cache_dir: ./model_cache    # 留空 = 默认 ~/.cache/huggingface（HF_HOME）
...
长期记忆检索默认走 **本地 embedding**（基于 [candle](https://github.com/huggingface/candle) 纯 Rust 推理），无需 API key、无网络往返，单次 CPU 推理 ~50-80ms。不依赖 ONNX Runtime，跨平台编译无 C++ 链接坑。可选模型：

| 模型 | 维度 | 体积 | 适用场景 |
|---|---|---|---|
| `bge-small-zh-v1.5`（默认） | 512 | ~100MB | 中文，体积/质量平衡 |
| `bge-large-zh-v1.5` | 1024 | ~1.2GB | 中文，质量更好 |
| `multilingual-e5-small` | 384 | ~120MB | 多语言 |
| `multilingual-e5-base` | 768 | ~470MB | 多语言，质量更好 |
| `multilingual-e5-large` | 1024 | ~2.2GB | 多语言，最强 |
| `bge-small-en-v1.5` | 384 | ~130MB | 纯英文 |
| `bge-base-en-v1.5` | 768 | ~440MB | 纯英文，质量更好 |

> BGE/E5 架构上就是 BERT + mean pooling + L2 normalize，candle 的 `BertModel` 直接支持。

**首次启动**会自动从 HuggingFace 下载 `config.json` / `tokenizer.json` / `model.safetensors` 到 HF 缓存目录（默认 `~/.cache/huggingface`，可设 `HF_HOME` 或 `embeddings.cache_dir` 覆盖），之后完全离线运行。
...
若镜像仍有问题（如 `Content-Range is missing` 报错），可手动用 git clone 把模型 repo 拉到本地再放置到 HF 缓存的标准布局：

```bash
# 1. 用镜像 git clone 整个 repo（含 tokenizer/config 等小文件；LFS 大文件可能仍需代理）
HF_ENDPOINT=https://hf-mirror.com git clone --depth 1 https://hf-mirror.com/BAAI/bge-small-zh-v1.5

# 2. 把文件放到 HF cache 的 snapshot 目录
#    路径形如 ~/.cache/huggingface/hub/models--BAAI--bge-small-zh-v1.5/snapshots/<commit>/
#    <commit> 见 ~/.cache/huggingface/hub/models--BAAI--bge-small-zh-v1.5/refs/main
```

也可以在能直连 HuggingFace 的环境（或挂代理）首次运行一次，把整个缓存目录拷贝到目标机器。

#### 存量数据迁移

从旧版（远程云端 embedding）升级时，启动会检测到 `memories` 表里的旧向量维度（1536 维）与新本地模型不兼容，自动清空 `memories` 表（打 `tracing::warn` 日志）并升级 schema 版本到 2。如有重要记忆请提前备份 `data/applepi.db`。
...
| `embeddings.model` | 本地 embedding 模型名 | `bge-small-zh-v1.5` |
| `embeddings.cache_dir` | 模型缓存目录 | 留空 = `~/.cache/huggingface`（HF_HOME） |
