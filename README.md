...
embeddings:
  model: bge-small-zh-v1.5      # 本地 embedding 模型（fastembed/ONNX，离线推理）
  # cache_dir: ./model_cache    # 留空 = 默认 ./.fastembed_cache
...
长期记忆检索默认走 **本地 embedding**（基于 [fastembed](https://github.com/Anush008/fastembed-rs) + ONNX Runtime），无需 API key、无网络往返，单次推理 <30ms。可选模型：
...
若镜像仍有问题（如 `Content-Range is missing` 报错），可手动用 git clone 把模型 repo 拉到本地再放置到 fastembed cache 的标准布局：

```bash
# 1. 用镜像 git clone 整个 repo（含 tokenizer 等小文件；LFS 大文件可能仍需代理）
HF_ENDPOINT=https://hf-mirror.com git clone --depth 1 https://hf-mirror.com/Xenova/bge-small-zh-v1.5

# 2. 把文件放到 fastembed cache 的 snapshot 目录
#    路径形如 .fastembed_cache/models--Xenova--bge-small-zh-v1.5/snapshots/<commit>/
#    <commit> 见 .fastembed_cache/models--Xenova--bge-small-zh-v1.5/refs/main
```

也可以在能直连 HuggingFace 的环境（或挂代理）首次运行一次，把整个 `.fastembed_cache/` 目录拷贝到目标机器。
...
| `embeddings.cache_dir` | 模型缓存目录 | 留空 = `./.fastembed_cache` |
